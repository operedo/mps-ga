...
!  integer(4), pointer   :: childC(:), childD(:)
  ...
  open (unit = 13, file = "../data/data_testcrossover2D.txt")
  read (13,*) number, rows, cols, tem_rows,tem_cols, cut_points
  close(13)
...
!  allocate(childD(rows*cols))
  write (*,*) "parentA:"
  write (*,'(10I1)')indivA%matrix
  write (*,*) "parentB:"
  write (*,'(10I1)')indivB%matrix

  call crossover2D(indivA%matrix,indivB%matrix,indivC%matrix,rows,cols,tem_rows,tem_cols,cut_points)

  write (*,*) "child:"
  write (*,'(10I1)')indivC%matrix
!  write (*,*) "childD:"
!  write (*,'(100I1)'),childD
