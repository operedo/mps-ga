program testfitnessChainOperations
  ...
  call loadHistogram(indivTI%matrix,rows,cols,tem_rows,tem_cols)

  print *,'TI histogram:'
  write(*,'(16I2)')chainArray(:)%frequency
  
  print *,'RE histogram:'

  call fitnessFunctionChainOperations(indivRE%matrix,rows,cols,tem_rows,tem_cols,value)
...
end program testfitnessChainOperations      
